...
static int certImport( const int certNo, const BOOLEAN isECC, 
	...
	if( status == CRYPT_ERROR_BADDATA && !( isECC || isBase64 ) && \
		...
		/* This certificate has a non-DER encoding of the serialNumber value
		   100, '02 04 00 00 00 64' */
		fputs( "Warning: Certificate import failed because the serial "
			   "number has an invalid\n         (non-DER) encoding.\n\n", 
			   outputStream );
		return( TRUE );
		}
	if( status == CRYPT_ERROR_BADDATA && !( isECC || isBase64 ) && \
		...
		/* This certificate has the algoID in the signature altered to make 
		   it invalid, since this isn't covered by the signature it isn't 
		   detected by many implementations */
	...
	if( status == CRYPT_ERROR_OVERFLOW && !( isECC || isBase64 ) && \
		...
		fprintf( outputStream, "Certificate import failed for certificate "
				 "with key larger than %d bits.\n", CRYPT_MAX_PKCSIZE * 8 );
		fputs( "  (This is the correct result for this test).\n\n", 
			   outputStream );
		return( TRUE );
		}
#endif /* Nonstandard CRYPT_MAX_PKCSIZE */
	if( cryptStatusError( status ) )
		{
...
static int certReqImport( const int certNo )
	...
	FILE *filePtr;
	BYTE buffer[ BUFFER_SIZE ];
	int count, complianceValue DUMMY_INIT, status;
...
static BOOLEAN handleCertChainError( const CRYPT_CERTIFICATE cryptCertChain, 
	...
	if( errorCode != CRYPT_ERROR_INVALID )
		{
		return( extErrorExit( cryptCertChain, "cryptCheckCert()", 
							  errorCode, __LINE__ ) );
		}
...
static int certChainImport( const int certNo, const BOOLEAN isBase64 )
	...
	if( cryptStatusError( status ) )
		{
		return( extErrorExit( cryptCertChain, "cryptGetAttribute()", 
							  status, __LINE__ ) );
		}
...
int testOCSPImport( void )
	...
	/* Now import the OCSP revoked responses.  These have a different CA 
	   certificate than the OK response, to keep things simple we don't 
	   bother with a sig check for them */
	for( i = 1; i <= 2; i++ )
		{
		fprintf( outputStream, "\nTesting OCSP revoked response %d "
				 "import...\n", i );
		filenameFromTemplate( buffer, OCSP_REV_FILE_TEMPLATE, i );
		if( ( filePtr = fopen( buffer, "rb" ) ) == NULL )
			{
			puts( "Couldn't find OCSP revoked response %d file for import "
				  "test." );
			return( FALSE );
			}
		count = fread( buffer, 1, BUFFER_SIZE, filePtr );
		fclose( filePtr );
		fprintf( outputStream, "OCSP revoked response %d has size %d "
				 "bytes.\n", i, count );
		status = cryptImportCert( buffer, count, CRYPT_UNUSED,
								  &cryptCert );
		if( cryptStatusError( status ) )
...
static int checkChain( const CHAINTEST_CERT_TYPE leftCertType, 
	...
	if( cryptStatusOK( status ) )
		...
		if( statusOK )
			{
			printExtError( leftCert, NULL, 0, 0 );
...
static const PATH_TEST_INFO pathTestInfoNIST[] = {
...
static int testPathNIST( const PATH_TEST_INFO *pathInfo )
	{
	...
	if( pathInfo->isValid )
		...
		if( cryptStatusError( status ) )
			...
			return( extErrorExit( cryptCertPath, "cryptCheckCert()",
								  status, __LINE__ ) );
			}
		}
	else
...
int testPathProcessingNIST( void )
	...
	fputs( "Testing NIST path processing...\n", outputStream );
	...
	/* Process each certificate path and make sure that it succeeds or fails 
	   as required */
	for( i = 0; pathTestInfoNIST[ i ].fileMajor != 0; i++ )
		{
		if( !testPathNIST( &pathTestInfoNIST[ i ] ) )
			break;
		}
	...
	cryptSetAttribute( CRYPT_UNUSED, CRYPT_OPTION_CERT_COMPLIANCELEVEL,
					   complianceLevel );
	if( pathTestInfoNIST[ i ].fileMajor )
		return( FALSE );
...
/****************************************************************************
*																			*
*							RS Path-processing Test							*
*																			*
****************************************************************************/

/* Test path processing using the RS PKI test suite */

static const PATH_TEST_INFO pathTestInfoRS[] = {
	/* 2.1.2.1. basicConstraints */
	/*   0 */ { 1, 1, FALSE },
	/*   1 */ { 1, 2, TRUE },
	/*   2 */ { 1, 3, FALSE },
	/*   3 */ { 1, 4, TRUE },
	...
	/* 2.1.2.9.1 subjectDirectoryAttributes clearance */
	/*  12 */ { 91, 1, FALSE },
	/*  13 */ { 91, 2, TRUE },
	/*  14 */ { 91, 3, TRUE },
	/*  15 */ { 91, 4, TRUE },
...
static int testPathRS( const PATH_TEST_INFO *pathInfo )
	{
	CRYPT_CERTIFICATE cryptCertPath;
	char pathName[ 64 ];
	int pathNo, requirePolicy, status;
	...
	/* Convert the composite path info into a single number used for fetching
	   the corresponding data file */
	sprintf( pathName, "212%d%d", pathInfo->fileMajor, pathInfo->fileMinor );
	pathNo = atoi( pathName );

	/* Test the path */
	sprintf( pathName, "2.1.2.%d.%d", pathInfo->fileMajor, pathInfo->fileMinor );
	fprintf( outputStream, "  Path %s%s...", pathName, 
			 pathInfo->policyOptional ? " without explicit policy" : "" );
	status = importCertFromTemplate( &cryptCertPath,
									 RSTEST_FILE_TEMPLATE, pathNo );
	if( cryptStatusError( status ) )
		{
		fprintf( outputStream, "Certificate import for test path %s failed, "
				 "status %d, line %d.\n", pathName, status, __LINE__ );
...
int testPathProcessingRS( void )
	...
	fputs( "Testing RS path processing...\n", outputStream );
...
int xxxCertCheck( const C_STR certFileName, const C_STR caFileNameOpt )
	...
	if( caFileNameOpt == NULL )
		...
		if( cryptStatusError( status ) )
			{
			fprintf( outputStream, "CA certificate '%s' import failed, "
...
int xxxCRLCheck( const C_STR certFileName, const C_STR crlFileName )
	...
	status = importCertFile( &cryptCRL, crlFileName );
	if( cryptStatusError( status ) )
		{
		fprintf( outputStream, "CRL import failed, status = %d.\n", 
				 status );
		cryptDestroyCert( cryptCert );
		return( FALSE );
		}
	...
	if( cryptStatusError( status ) )
		...
		if( status == CRYPT_ERROR_INVALID )
			{
			fprintf( outputStream, "Certificate has been revoked, "
					 "status = %d.\n", status );
			}
		else
			{
