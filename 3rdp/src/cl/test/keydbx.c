...
static const struct {
	...
	{ TEXT( "ldap.sk.ee" ), "ldap.sk.ee",
	  TEXT( "CN=EID-SK 2011, OU=Sertifitseerimisteenused, O=AS Sertifitseerimiskeskus, C=EE" ) }
	};

#define LDAP_SERVER_NO		1
#define LDAP_ALT_SERVER_NO	2	/* Secondary svr.if main server unavailable */
#define LDAP_ALT2_SERVER_NO	4	/* Another svr.if previous ones unavailable */
...
/* Create a certificate with fields designed to cause problems for some 
   keysets */
...
static int createTestCertificate( CRYPT_CERTIFICATE *cryptCert,
								  const time_t validFrom )
	{
...
static int testKeysetRead( const CRYPT_KEYSET_TYPE keysetType,
	...
	if( option == READ_OPTION_MULTIPLE )
		...
		for( i = 0; i < 3; i++ )
			...
			if( cryptStatusError( status ) )
				{
				printf( "cryptGetPublicKey() with cached query failed on "
						"cert %d with error code %d, line %d.\n", i + 1, 
						status, __LINE__ );
				return( FALSE );
				}
			cryptDestroyCert( cryptCert );
...
static int testKeysetWrite( const CRYPT_KEYSET_TYPE keysetType,
	...
	if( useAltCert )
		...
		status = getPublicKey( &cryptCert, filenameBuffer, 
							   USER_PRIVKEY_LABEL );
		}
	else
		{
	...
	if( status == CRYPT_ERROR_DUPLICATE )
		{
		printf( "Certificate database '%s' already exists, reusing existing "
				"database.\n", keysetName );
		status = cryptKeysetOpen( &cryptKeyset, CRYPT_UNUSED, keysetType,
	...
	/* Add a third certificate with a UTF-8 component which, when turned into
	   Unicode, exceeds CRYPT_MAX_TEXTSIZE */
	if( keysetType != CRYPT_KEYSET_LDAP )
		{
		status = importCertFromTemplate( &cryptCert, CERT_FILE_TEMPLATE, 36 );
		if( cryptStatusError( status ) )
			{
			printf( "Couldn't read certificate from file, status %d, "
					"line %d.\n", status, __LINE__ );
	...
	/* Add a fourth certificate with a DN that'll cause problems for some 
	   storage technologies */
	if( !createTestCertificate( &cryptCert, 0 ) )
		return( FALSE );
	status = cryptAddPublicKey( cryptKeyset, cryptCert );
	if( cryptStatusError( status ) )
		{
		/* The certificate is already present, delete it and retry the 
		   write */
		status = cryptGetAttributeString( cryptCert,
								CRYPT_CERTINFO_COMMONNAME, name, &length );
		if( cryptStatusOK( status ) )
	...
	/* Re-add a copy of the fourth certificate with a newer validity time, 
	   which tests the ability to update an existing certificate, then try 
	   adding it again to make sure that it's detected as a duplicate.  This 
	   gets a bit complicated because we can't guarantee that sufficient 
	   time has passed between creating the previous copy of the certificate 
	   and this one that it'll be regarded as newer, so we explicitly set 
	   its start date to one second newer than the previous one */
	if( !createTestCertificate( &cryptCert, validFrom + 1 ) )
		return( FALSE );
	status = cryptAddPublicKey( cryptKeyset, cryptCert );
	if( cryptStatusError( status ) )
		{
		return( extErrorExit( cryptKeyset, 
							  "cryptAddPublicKey() of newer replacement "
							  "certificate", status, __LINE__ ) );
		}
	status = cryptAddPublicKey( cryptKeyset, cryptCert );
	if( status != CRYPT_ERROR_DUPLICATE )
		{
		printf( "Addition of duplicate certificate wasn't detected, "
				"line %d.\n", __LINE__ );
		return( FALSE );
		}
	cryptDestroyCert( cryptCert );
...
static int testQuery( const CRYPT_KEYSET_TYPE keysetType, const C_STR keysetName )
	{
	CRYPT_KEYSET cryptKeyset;
	int count = 0, status;
...
int testReadCert( void )
	...
	status = testKeysetRead( CRYPT_KEYSET_DATABASE, DATABASE_KEYSET_NAME,
...
int testWriteCert( void )
	...
	status = testKeysetWrite( CRYPT_KEYSET_DATABASE, DATABASE_KEYSET_NAME, 
							  FALSE );
	if( status == CRYPT_ERROR_NOTAVAIL )
		{
		/* Database keyset access not available */
...
#ifdef __UNIX__
		/* Many Unix systems have ODBC libraries installed by default 
		   without any ODBC data sources being set up, which means they look
		   like a failed ODBC keyset open.  To avoid aborting the self-test
		   with a false-positive fault we report an open error under Unix
		   as a not-available */
		return( CRYPT_ERROR_NOTAVAIL );
#else
		return( FALSE );
#endif /* Systems likely to have ODBC installed */
		}
	if( !status )
		return( FALSE );
	status = testKeysetWrite( CRYPT_KEYSET_DATABASE, DATABASE_KEYSET_NAME, 
...
int testKeysetQuery( void )
	...
	status = testQuery( CRYPT_KEYSET_DATABASE, DATABASE_KEYSET_NAME );
	if( status == CRYPT_ERROR_NOTAVAIL )
		{
		/* Database keyset access not available */
		return( CRYPT_ERROR_NOTAVAIL );
...
int testReadCertLDAP( void )
	...
	int ldapServerNo = LDAP_SERVER_NO;
	int length, status;
	...
	if( status == CRYPT_ERROR_OPEN )
		...
		printf( "%s not available for some odd reason,\n  trying "
				...
				ldapUrlInfo[ ldapServerNo ].asciiURL );
		ldapServerNo = LDAP_ALT_SERVER_NO;
		ldapKeysetName = ldapUrlInfo[ ldapServerNo ].url;
	...
	if( status == CRYPT_ERROR_OPEN )
		{
		printf( "%s also not available, trying\n  yet another "
				"alternative directory instead...\n", 
				ldapUrlInfo[ ldapServerNo ].asciiURL );
		ldapServerNo = LDAP_ALT2_SERVER_NO;
		ldapKeysetName = ldapUrlInfo[ ldapServerNo ].url;
		status = cryptKeysetOpen( &cryptKeyset, CRYPT_UNUSED,
								  CRYPT_KEYSET_LDAP, ldapKeysetName,
								  CRYPT_KEYOPT_READONLY );
