...
static int certProcess( const CRYPT_ALGO_TYPE cryptAlgo,
	...
	const char *certName = \
			( cryptAlgo == CRYPT_ALGO_RSA ) ? \
				( useCRMF ? "pr_cert_rsa_crmf" : "pr_cert_rsa" ) : \
			( cryptAlgo == CRYPT_ALGO_DSA ) ? "pr_cert_dsa" : \
			( cryptAlgo == CRYPT_ALGO_DH ) ? "pr_cert_dh" : \
...
int testCertProcess( void )
	...
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_RSA, NULL ) ) && \
		!certProcess( CRYPT_ALGO_RSA, "RSA", cryptCAKey, FALSE ) )
		return( FALSE );
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_DSA, NULL ) ) && \
		!certProcess( CRYPT_ALGO_DSA, "DSA", cryptCAKey, FALSE ) )
		return( FALSE );
	...
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_DH, NULL ) ) && \
		!certProcess( CRYPT_ALGO_DH, "Diffie-Hellman", cryptCAKey, FALSE ) )
		return( FALSE );
	...
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_RSA, NULL ) ) && \
		!certProcess( CRYPT_ALGO_RSA, "RSA", cryptCAKey, TRUE ) )
		return( FALSE );
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_ECDSA, NULL ) ) && \
		!certProcess( CRYPT_ALGO_ECDSA, "ECDSA", cryptCAKey, TRUE ) )
		return( FALSE );
	...
	if( cryptStatusOK( cryptQueryCapability( CRYPT_ALGO_RSA, NULL ) ) )
		...
		status = certProcess( CRYPT_ALGO_RSA, 
							  "RSA with non-default hash algorithm", 
							  cryptCAKey, FALSE );
...
static int addCertRequest( const CRYPT_KEYSET cryptCertStore,
	...
	if( cryptStatusError( status ) )
		{
		return( extErrorExit( cryptCertRequest, "cryptSetAttribute()",
							  status, __LINE__ ) );
		}
	...
	if( cryptStatusError( status ) )
		{
		return( extErrorExit( cryptCertRequest, "cryptSignCert()",
							  status, __LINE__ ) );
		}
