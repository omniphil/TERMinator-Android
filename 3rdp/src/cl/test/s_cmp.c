...
typedef enum {
	PNPPKI_TEST_NONE,		/* No PnPPKI test type */
	PNPPKI_TEST_NORMAL,		/* Standard PnPPKI test */
	PNPPKI_TEST_PKIBOOT,	/* PKIBoot PnPPKI test */
	PNPPKI_TEST_CA,			/* CA PnPPKI test */
	PNPPKI_TEST_INTERMED_CA,/* Intermediate CA PnPPKI test */
	PNPPKI_TEST_DEVICE,		/* Crypto device PnPPKI test */
	PNPPKI_TEST_LAST		/* Last possible PnpPKI test type */
	} PNPPKI_TEST_TYPE;
...
/* There are various CMP test CAs available, the following mappings can be
   used to test different ones.  Implementation peculiarities:
	...
	#13 - EJBCA: Implements a broken version of pre-RFC CMPv1, can't be 
			tested.
	#14 - EJBCA: Newer than #13, sends unauthenticated error response in case
			of problems.
	#15 - EJBCA: Variant of #14, requires additional certificates in 
			extraCerts field as per ETSI 33.310.
			
   Of the above, EJBCA handles users/entities as single-use, to deal with 
   this we allow EJBCA user names to be changed on each run */

#define EJBCA_USERNAME			"dtsid811"	/* Available: 810-813 */
...
static const CA_INFO caInfoTbl[] = {
	...
	{ /*  2 */ "cryptlib/PKIBoot", /*"_pkiboot._tcp.cryptoapps.com"*/
				TEXT( "http://" LOCAL_HOST_NAME ), TEXT( "interop" ), TEXT( "interop" ) },
	{ /*  3 */ "Certicom", TEXT( "cmp://gandalf.trustpoint.com:8081" ), 
				TEXT( "interop" ), TEXT( "interop" ) },
	{ /*  4 */ "ssh", TEXT( "cmp://interop-ca.ssh.com:8290" ), 
				TEXT( "123456" ), TEXT( "interop" ) },
	{ /*  5 */ "ssh", TEXT( "http://pki.ssh.com:8080/pkix/" ), 
				TEXT( "62154" ), TEXT( "ssh" ) },
	{ /*  6 */ "Entrust", TEXT( "cmp://204.101.128.45:829" ), 
				TEXT( "39141091" ), TEXT( "ABCDEFGHIJK" ) },
	{ /*  7 */ "Trustcenter", TEXT( "cmp://demo.trustcenter.de/cgi-bin/cmp:829" ), 
				TEXT( "interop" ), TEXT( "interop" ) },
	{ /*  8 */ "Baltimore", TEXT( "cmp://hip.baltimore.ie:8290" ), 
				TEXT( "pgutmann" ), TEXT( "the-magical-land-near-oz" ) },
	{ /*  9 */ "Initech", TEXT( "cmp://61.74.133.49:8290" ), 
				TEXT( "interop" ), TEXT( "interop" ) },
	{ /* 10 */ "RSA", TEXT( "cmp://ca1.kcspilot.com:32829" ), 
				TEXT( "interop" ), TEXT( "interop" ) },
	{ /* 11 */ "Cylink", TEXT( "cmp://216.252.217.227:8082" ), 
				TEXT( "3986" ), TEXT( "11002" ) /* "3987", "6711" */ },
	{ /* 12 */ "Insta-Certifier", TEXT( "http://pki.certificate.fi:8700/pkix/" ), 
				TEXT( "3078" ), TEXT( "insta" ) },
	{ /* 13 */ "EJBCA", TEXT( "http://192.168.1.103:8080/ejbca/publicweb/cmp" ), 
				TEXT( EJBCA_USERNAME ), TEXT( "password" ) },
	{ /* 14 */ "EJBCA", TEXT( "http://ec2-15-188-87-0.eu-west-3.compute.amazonaws.com:8080/ejbca/publicweb/cmp/pocCMPalias" ), 
				TEXT( EJBCA_USERNAME ), TEXT( "12345678901234567890" ) },
 	{ /* 15 */ "EJBCA", TEXT( "http://ec2-15-188-87-0.eu-west-3.compute.amazonaws.com:8080/ejbca/publicweb/cmp/pocCMPalias2" ),
				TEXT( EJBCA_USERNAME ), TEXT( "12345678901234567890" ) }
	};
...
/* Workarounds for CA bugs/quirks */
...
#if ( CA_NO == 9 )							/* Initech */
  #define SERVER_FIXED_DN_1
#elif ( CA_NO == 14 ) || ( CA_NO == 15 )	/* EJBCA */
  #define SERVER_FIXED_DN_2
#endif /* CAs that require a fixed DN in requests */
#if ( CA_NO == 14 ) || ( CA_NO == 15 )		/* EJBCA */
  #define SERVER_CUSTOM_KEYSIZE		256
#endif /* CAs that require a specific key size */
...
#elif ( CA_NO == 14 ) || ( CA_NO == 15 )
  #define TEST_IR
  #define TEST_KUR

  /* Turn unused functionality into no-ops */
...
static const CERT_DATA cmpRsaSignRequestData[] = {		/* For cr */
  ...
  #elif defined( SERVER_FIXED_DN_2 )
	{ CRYPT_CERTINFO_COUNTRYNAME, IS_STRING, 0, TEXT( "FR" ) },
	{ CRYPT_CERTINFO_ORGANIZATIONNAME, IS_STRING, 0, TEXT( "GRDF" ) },
...
static int createCmpNewKeyRequest( const CERT_DATA *requestData,
	...
	{
	CRYPT_CERTIFICATE cryptRequest DUMMY_INIT;
...
#ifdef SERVER_CUSTOM_KEYSIZE
		cryptSetAttribute( cryptContext, CRYPT_CTXINFO_KEYSIZE, 
						   SERVER_CUSTOM_KEYSIZE );
#endif /* SERVER_CUSTOM_KEYSIZE */
...
static int createCmpSession( const CRYPT_CONTEXT cryptCACert,
	...
	if( cryptStatusOK( status ) && options != CRYPT_CMPOPTION_NONE )
		{
		status = cryptSetAttribute( cryptSession, 
									CRYPT_SESSINFO_CMP_OPTIONS, options );
		}
	if( cryptStatusError( status ) )
		{
...
#if defined( SERVER_IS_CRYPTLIB ) || defined( SERVER_FIXED_DN_1 ) || \
	defined( SERVER_FIXED_DN_2 )
...
static int revokeCert( const char *description, const CA_INFO *caInfoPtr,
	...
	if( signRequest )
		...
		cryptSession = createCmpSession( cryptCACert, caInfoPtr->url,
										 ...
										 CRYPT_CMPOPTION_NONE, CRYPT_UNUSED );
		}
	if( cryptSession <= 0 )
		return( cryptSession );
...
/* Test the full range of CMP functionality.  This performs the following
   tests:
	...
	RSA sign:
		ir + ip + reject (not performed since it requires a cmp.c mod)
	...
	RSA encr.:
		ir + ip + reject (as above, requires a cmp.c mod)
		ir + ip + certconf + pkiconf
		rr + rp (of ir certificate)
	DSA/ECDSA:
...
static int connectCryptlibCMP( const CMP_TEST_TYPE testType,
	...
	if( requestCert( "Duplicate init.request", &caInfo, NULL, NULL,
					 ...
					 FALSE, TRUE, CRYPT_CMPOPTION_NONE, NULL ) )
		{
	...
	status = requestCert( "DSA certificate", &caInfo, readFileName, NULL,
						  ...
						  FALSE, FALSE, CRYPT_CMPOPTION_NONE, NULL );
	if( status != TRUE )
		return( status );
	delayThread( 2 );		/* Wait for server to recycle */
#endif /* 0 */
...
static int connectCMP( const CRYPT_ALGO_TYPE cryptAlgo )
...
#if ( CA_NO == 13 ) || ( CA_NO == 14 ) || ( CA_NO == 15 )
	const int options = CRYPT_CMPOPTION_3GPP;
#else
	const int options = CRYPT_CMPOPTION_NONE;
#endif /* 3GPP */
	int status;
	...
	/* Encryption-only certificate request.  This test requires a change in
	   ...
	   to disallow this.  Disallowing signing for encryption-only keys would 
	   break PKCS #10 since it's then no longer possible to create the self-
	   signed request, this is a much bigger concern than CMP.  
	   
	   Note that this functionality is tested in any case by the PnP PKI 
	   test, which creates the necessary encryption-only requests internally 
	   and can do things that we can't do from the outside */
	status = requestCert( "encryption-only certificate", caInfoPtr,
						  ...
						  cryptCACert, FALSE, FALSE, options, NULL );
	if( status != TRUE )
		return( status );
#endif /* 0 */
...
/* Connect to a non-cryptlib CMP server with 3GPP functionality */

static int connectCMP3GPP( void )
	{
	CRYPT_CERTIFICATE cryptCACert;
	C_CHR readFileName[ FILENAME_BUFFER_SIZE ];
	C_CHR writeFileName[ FILENAME_BUFFER_SIZE ];
	...
	/* Get the certificate of the CA who will issue the certificate.  This 
	   is a root CA certificate rather than a certificate chain ending at
	   an intermediate CA, since the CMP server will use an intermediate CA 
	   to issue the certificate we need to use the 3GPP variant of CMP to 
	   retrieve the intermediate CA certificates as part of the protocol */
	status = importCertFromTemplate( &cryptCACert, CMP_CA_FILE_TEMPLATE,
									 CA_NO );
	if( cryptStatusError( status ) )
		{
	...
	/* Get the certificate using the 3GPP variant of CMP.  We have to 
	   perform an ir first to set up a certificate with most of the DN data
	   supplied by the CA since otherwise the DN is fixed and we can only
	   perform a kur */
	status = getPkiUserInfo( TEXT( "Procurement" ), &caInfo, userID, 
							 issuePW, revPW );
	if( status != TRUE )
...
static int cmpServerSingleIteration( const CRYPT_CONTEXT cryptPrivateKey,
	...
	if( cryptStatusOK( status ) && options != CRYPT_CMPOPTION_NONE )
		{
		status = cryptSetAttribute( cryptSession, CRYPT_SESSINFO_CMP_OPTIONS, 
									options );
		}
	if( cryptStatusError( status ) )
		{
...
static int cmpServer3GPP( void )
	...
	puts( "SVR: Testing CMP server for 3GPP processing..." );

	/* Set up the server-side objects.  Since we're working with an
	   intermediate CA that requires a certificate chain to exercise the 
	   peculiar 3GPP certificate handling, we use the intermediate-CA key 
	   rather than the root CA key */
	if( !pkiServerInit( &cryptCAKey, &cryptCertStore, ICA_PRIVKEY_FILE,
						USER_PRIVKEY_LABEL, cmpPkiUserFullDNData, 
						cmpPkiUserPartialDNData, cmpPkiUserCaData, 
						cmpPkiUserRaData, "CMP" ) )
		return( FALSE );
	...
	/* Run the server twice, first to handle the ir to set up a certificate
	   for the 3GPP-profile cr, and then again to handle the 3GPP-profile 
	   cr */
	puts( "SVR: Running server for 3GPP-profile pre-certificate issue." );
	if( !cmpServerSingleIteration( cryptCAKey, cryptCertStore, FALSE, 
								   CRYPT_CMPOPTION_3GPP ) )
		{
		puts( "SVR: 3GPP CMP pre-certificate issue failed.\n" );
		return( FALSE );
		}
	puts( "SVR: Running server for 3GPP-profile certificate issue (cr)." );
	if( !cmpServerSingleIteration( cryptCAKey, cryptCertStore, FALSE, 
								   CRYPT_CMPOPTION_3GPP ) )
		{
		puts( "SVR: 3GPP CMP certificate issue (cr) failed.\n" );
		return( FALSE );
		}
	...
	puts( "SVR: CMP 3GPP processing successful.\n" );
	return( TRUE );
	}
...
static int pnppkiServer( const PNPPKI_TEST_TYPE testType )
	{
	CRYPT_CONTEXT cryptCAKey;
	CRYPT_KEYSET cryptCertStore;
	...
	printf( "SVR: Testing %s server session%s...\n",
			( testType == PNPPKI_TEST_PKIBOOT ) ? \
			  "PKIBoot" : "plug-and-play PKI",
			( testType == PNPPKI_TEST_CA ) ? \
			  " for CA certificate" : \
			( testType == PNPPKI_TEST_INTERMED_CA ) ? \
			  " using intermediate CA" : "" );

	/* Get the information needed by the server */
	if( testType == PNPPKI_TEST_INTERMED_CA )
		{
		/* The intermediate CA has a PnP-generated key so the key label is
	...
	if( !cmpServerSingleIteration( cryptCAKey, cryptCertStore, 
								   ( testType == PNPPKI_TEST_DEVICE ) ? \
									 TRUE : FALSE, CRYPT_CMPOPTION_NONE ) )
		return( FALSE );
  ...
  static unsigned __stdcall cmpServerThread( void *dummy )
#else
  static void *cmpServerThread( void *dummy )
...
static int cmpClientServer( const CMP_TEST_TYPE testType,
	...
	hThread = ( HANDLE ) _beginthreadex( NULL, 0, cmpServerThread,
										 NULL, 0, &threadID );
#else
	pthread_create( &hThread, NULL, cmpServerThread, NULL );
#endif /* Windows vs. Unix threads */
	THREAD_SLEEP( 1000 );
...
int testSessionCMPSHA2ClientServer( void )
	...
	/* Run the test with the changed algorithm */
	status = cmpClientServer( CMP_TEST_NORMAL, CRYPT_ALGO_RSA );
...
#ifdef WINDOWS_THREADS
  static unsigned __stdcall pnpPkiServerThread( void *arg )
#else
  static void *pnpPkiServerThread( void *arg )
#endif /* Windows vs. Unix threads */
	{
	const int argValue = *( ( int * ) arg );

	pnppkiServer( argValue );
	THREAD_EXIT();
	}

static int pnpPkiClientServer( const PNPPKI_TEST_TYPE testType )
	{
	THREAD_HANDLE hThread;
#ifdef __WINDOWS__
	unsigned threadID;
	...
	createMutex();
#ifdef WINDOWS_THREADS
	hThread = ( HANDLE ) _beginthreadex( NULL, 0, pnpPkiServerThread, &arg, 0, 
										 &threadID );
#else
	pthread_create( &hThread, NULL, pnpPkiServerThread, &arg );
#endif /* Windows vs. Unix threads */
	THREAD_SLEEP( 1000 );

	/* Connect to the local server as appropriate  */
	switch( testType )
		{
		case PNPPKI_TEST_PKIBOOT:
			status = connectCryptlibCMP( CMP_TEST_PKIBOOT, CRYPT_ALGO_RSA );
			break;

		case PNPPKI_TEST_NORMAL:
		case PNPPKI_TEST_INTERMED_CA:
			status = connectPNPPKI( FALSE, FALSE, TRUE );
			break;

		case PNPPKI_TEST_DEVICE:
...
int testSessionCMPPKIBootClientServer( void )
	{
	return( pnpPkiClientServer( PNPPKI_TEST_PKIBOOT ) );
	}
int testSessionPNPPKIClientServer( void )
	{
	return( pnpPkiClientServer( PNPPKI_TEST_NORMAL ) );
	}
int testSessionPNPPKIDeviceClientServer( void )
	{
	return( pnpPkiClientServer( PNPPKI_TEST_DEVICE ) );
	}
int testSessionPNPPKICAClientServer( void )
	{
	return( pnpPkiClientServer( PNPPKI_TEST_CA ) );
	}
int testSessionPNPPKIIntermedCAClientServer( void )
	{
	return( pnpPkiClientServer( PNPPKI_TEST_INTERMED_CA ) );
	}

#ifdef WINDOWS_THREADS
  static unsigned __stdcall cmpServerRAThread( void *dummy )
#else
  static void *cmpServerRAThread( void *dummy )
#endif /* Windows vs. Unix threads */
	{
	cmpServerRA();
	THREAD_EXIT();
...
int testSessionCMPRAClientServer( void )
	...
	hThread = ( HANDLE ) _beginthreadex( NULL, 0, cmpServerRAThread,
										 NULL, 0, &threadID );
#else
	pthread_create( &hThread, NULL, cmpServerRAThread, NULL );
#endif /* Windows vs. Unix threads */
	THREAD_SLEEP( 1000 );
...
#ifdef WINDOWS_THREADS
  static unsigned __stdcall cmpServer3GPPThread( void *dummy )
#else
  static void *cmpServer3GPPThread( void *dummy )
#endif /* Windows vs. Unix threads */
	{
	cmpServer3GPP();
	THREAD_EXIT();
...
int testSessionCMP3GPPClientServer( void )
	...
	createMutex();
#ifdef __WINDOWS__
	hThread = ( HANDLE ) _beginthreadex( NULL, 0, cmpServer3GPPThread,
										 NULL, 0, &threadID );
#else
	pthread_create( &hThread, NULL, cmpServer3GPPThread, NULL );
#endif /* Windows vs. Unix threads */
	THREAD_SLEEP( 1000 );

	/* Connect to the local server with RA functionality */
	status = connectCMP3GPP();
	waitForThread( hThread );
	destroyMutex();
	return( status );
	}
...
#ifdef WINDOWS_THREADS
  static unsigned __stdcall cmpFailServerThread( void *dummy )
#else
  static void *cmpFailServerThread( void *dummy )
#endif /* Windows vs. Unix threads */
	{
	cmpServerFail();
	THREAD_EXIT();
...
int testSessionCMPFailClientServer( void )
	...
	hThread = ( HANDLE ) _beginthreadex( NULL, 0, cmpFailServerThread,
										 NULL, 0, &threadID );
#else
	pthread_create( &hThread, NULL, cmpFailServerThread, NULL );
#endif /* Windows vs. Unix threads */
	THREAD_SLEEP( 1000 );
