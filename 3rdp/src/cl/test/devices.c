...
/* Various features can be disabled by configuration options, in order to 
   ...
   In addition config.h can modify some settings in cryptlib.h if custom 
   profiles are being used, so we need to include it before we include
   cryptlib.h */
#include "misc/config.h"	/* For algorithm usage and custom profiles */
...
/* Which devices we want to test.  We always test PKCS #11 and TPMs (if 
   device tests are enabled), but this can be overridden if we're using 
   custom crypto hardware for our crypto, since some of the PKCS #11/TPM 
   mechanisms are dependent on built-in software crypto */

#if defined( CONFIG_CRYPTO_HW1 ) || defined( CONFIG_CRYPTO_HW2 )
  #if defined( _MSC_VER ) || defined( __GNUC__ ) || defined( __clang__ )
	#pragma message( "  Building for hardware device testing instead of PKCS #11." )
...
#ifdef TEST_KEYGEN
  #define DEVICE_PKC_ALGO_CRYPTOAPI			CRYPT_ALGO_RSA
  #define DEVICE_PKC_ALGO_TPM				CRYPT_ALGO_RSA
  #define DEVICE_PKC_ALGO_HARDWARE			CRYPT_ALGO_ECDSA
  #define DEVICE_PKC_ALGO_PKCS11			CRYPT_ALGO_RSA
#endif /* TEST_KEYGEN */
...
/* Device information tables for PKCS #11 device types.  This lists all the
   ...
   'ctinit -ptest' (sets the PIN to "test", but see further on) otherwise 
   the device will end up in a state where any attempt to use an object 
   results in a CKR_KEY_HANDLE_INVALID (so even a C_CreateObject() 
   followed immediately by a C_EncryptInit() using the handle returned from 
   C_CreateObject() returns CKR_KEY_HANDLE_INVALID).  Note that as of 
   Windows 10 the label is set to the default of "Alice", requiring an
   override 'ctinit -ptest -l"Software Only"' (the table below contains an
   entry for the default config with the "Alice" label alongside the one 
   set by the init process described here).
   ...
   The Netscape soft-token (softokn3.dll/libsoftokn3.so, i.e. NSS) is a bit 
   problematic to use, it requires the presence of three additional 
   libraries (nspr4.dll, plc4.dll, and plds4.dll in older versions, nss3.dll
   in newer versions) in the same directory, and even then requires that 
   C_Initialize() be called with proprietary and only partially-documented 
   nonstandard arguments, see the comment in device/pkcs11_init.c for more 
   on this.  The easiest way to arrange to have all the files in the right 
   place is to chdir to "C:/Program Files/Mozilla Firefox" before loading 
   the DLL, but even this doesn't mean that it'll work properly (this 
   behaviour isn't really a bug, softokn3.dll/.so was only ever meant to be 
   used as a crypto layer for NSS, it was never intended to be a general-
...
static const DEVICE_CONFIG_INFO pkcs11DeviceInfo[] = {
	...
	{ "Alice", "Eracom 2.x soft-token default config", "9999", "Test user key" },
	...
	{ "NSS Internal Crypto Services", "Netscape", "test", "Test user key" },
...
/* Device information for system TPM */

static const DEVICE_CONFIG_INFO tpmDeviceInfo[] = {
#if 1	/* Implicit device selection */
	{ "[Autodetect]", "Automatically detect device", "test", "Test user key" },
#else	/* Explicit device selection */
...
static BOOLEAN createKey( const CRYPT_DEVICE cryptDevice,
	...
	const CERT_DATA *certData = ( isCA ) ? cACertData : \
			...
			( cryptAlgo == CRYPT_ALGO_DSA || \
			  cryptAlgo == CRYPT_ALGO_ECDSA) ? userSigOnlyCertData : \
	...
	if( status == CRYPT_ERROR_NOTINITED )
		...
		if( willInitialise )
			...
			if( !strcmp( tokenLabel, "Software Only" ) || \
				...
				{
				printf( "\nThis token type can't be initialised in software "
						"but must be initialised via\nthe vendor-provided "
						"utility, cryptlib won't initialise the device.\n" );
				return( NULL );
				}
			return( deviceInfo );
			}
...
static BOOLEAN initialiseDevice( const CRYPT_DEVICE cryptDevice,
								 ...
								 const DEVICE_CONFIG_INFO *deviceInfo )
	{
	const char *defaultSSOPIN = deviceInfo->password;
	const char *ssoPIN = deviceInfo->password;
	const char *userPIN = deviceInfo->password;
	int status;

	/* PKCS #11 doesn't distinguish between zeroisation and initialisation,
	   so we only perform the zeroise test if it's built-in hardware.  This 
	   doesn't really distinguish between the two either but we perform the 
	   separate zeroise/initialise to make sure that the two work as 
	   required */
...
static BOOLEAN testPersistentObject( const CRYPT_DEVICE cryptDevice )
	{
	CRYPT_ALGO_TYPE cryptAlgo = CRYPT_ALGO_3DES;
	CRYPT_CONTEXT cryptContext;
	int status;
...
#if ( DEFAULT_CRYPT_ALGO != CRYPT_ALGO_AES )
	if( cryptStatusError( status ) )
		{
		cryptAlgo = DEFAULT_CRYPT_ALGO;
		status = cryptDeviceQueryCapability( cryptDevice, cryptAlgo, NULL );
		}
#endif /* DEFAULT_CRYPT_ALGO != CRYPT_ALGO_AES */
	if( status == CRYPT_ERROR_NOTAVAIL )
		{
		/* Some PKC-only devices don't support symmetric encryption so we 
		   just skip the test */
		printf( "\nDevice doesn't support 3DES, AES, or HMAC-SHA1, skipping "
...
static BOOLEAN testDeviceHighlevel( const CRYPT_DEVICE cryptDevice,
	...
	if( !isWriteProtected )
		...
		const CRYPT_ALGO_TYPE cryptAlgo = \
				( deviceType == CRYPT_DEVICE_CRYPTOAPI ) ? \
				  DEVICE_PKC_ALGO_CRYPTOAPI : \
				( deviceType == CRYPT_DEVICE_TPM ) ? \
				  DEVICE_PKC_ALGO_TPM : \
				( deviceType == CRYPT_DEVICE_HARDWARE ) ? \
				  DEVICE_PKC_ALGO_HARDWARE : \
				  DEVICE_PKC_ALGO_PKCS11;
		...
		if( deviceType != CRYPT_DEVICE_CRYPTOAPI )
			...
			/* CryptoAPI can only store one private key per provider so we
			   ...
			   certificate */
			status = getCAPrivateKey( &sigKeyContext, FALSE );
			}
		if( cryptStatusError( status ) )
			{
	...
	if( deviceType == CRYPT_DEVICE_PKCS11 )
		{
		/* If we're using a custom crypto HAL to test the PKCS #11 
		   operations then the following test will throw an exception due to
		   the (valid) PKCS #11 decrypt trying to decrypt garbage from the 
		   (emulated) HAL */
		puts( "The following test uses cryptlib native crypto to verify the "
			  "operation of\nthe crypto in the device.  Since a custom "
			  "crypto HAL is in use, the\ntest will fail if the HAL "
			  "emulates, but doesn't actually implement, the\nrequired "
			  "algorithm.\n" );
		}
...
/*	KLUDGE_WARN( "write-protect status" );
	...
	if( deviceType != CRYPT_DEVICE_TPM )
		...
		{
		puts( "Skipping TPM algorithm tests since direct key loads aren't "
			  "supported." );
...
int testDevices( void )
	...
	/* Test TPM device */
#ifdef TEST_TPM
	for( i = 0; tpmDeviceInfo[ i ].name != NULL; i++ )
		{
		status = testCryptoDevice( CRYPT_DEVICE_TPM, "TPM",
								   &tpmDeviceInfo[ i ] );
		if( cryptStatusError( status ) && \
			!( status == CRYPT_ERROR_NOTAVAIL || \
			   ( i == 0 && status == CRYPT_ERROR_WRONGKEY ) ) )
...
#define CLIENT_DEVICE_TYPE		CRYPT_DEVICE_HARDWARE
#define SERVER_DEVICE_TYPE		CRYPT_DEVICE_HARDWARE
...
static int openCertStore( CRYPT_KEYSET *cryptCertStore )
	...
	*cryptCertStore = -1;
	...
	status = cryptKeysetOpen( cryptCertStore, CRYPT_UNUSED,
							  CRYPT_KEYSET_DATABASE_STORE, "testcertstore",
							  CRYPT_KEYOPT_CREATE );
...
static int initUserInfo( const CRYPT_KEYSET cryptCertStore,
	...
	if( cryptStatusError( status ) )
		{
		return( extErrorExit( cryptPKIUser, "cryptSetAttributeString()",
							  status, __LINE__ ) );
		}
	puts( " done." );
...
static int pnpServer( const CRYPT_DEVICE cryptDevice,
	...
	( void ) cryptGetAttribute( cryptPrivateKey, 
