...
typedef enum {
	...
	/* Session faults.  The different between CORRUPT_DATA/MAC and 
	   BADSIG_DATA/MAC is that the former is for session payload data in TLS
	...
	/* TLS protocol-specific faults */
	FAULT_SESSION_TLS_CORRUPT_FINISHED,	/* Corruption of finished MAC */
	FAULT_SESSION_TLS_CORRUPT_IV,	/* Corruption of data IV */
...
#define FAULTACTION_SESSION_CORRUPT_HANDSHAKE_TLS_1 \
		faultParam1 = ( stell( stream ) - handshakeInfo->sessionIDlength ) + 4; \
		sessionInfoPtr->sendBuffer[ faultParam1 ]++
#define FAULTACTION_SESSION_CORRUPT_HANDSHAKE_TLS_2 \
		if( cryptStatusOK( status ) ) \
			sessionInfoPtr->sendBuffer[ faultParam1 ]--

/* Corrupt the payload data and MAC */

#define FAULTACTION_SESSION_CORRUPT_DATA_TLS_1 \
		if( !cryptStatusError( status ) ) \
			sessionInfoPtr->sendBuffer[ TLS_HEADER_SIZE + 4 ]++
#define FAULTACTION_SESSION_CORRUPT_MAC_TLS_1 \
		( ( BYTE * ) data )[ payloadLength + 2 ]++

/* Swap the server certificate for a different one */

#define FAULTACTION_SESSION_WRONGCERT_TLS_1 \
		faultParam1 = sessionInfoPtr->privateKey; \
		status = getSubstituteKey( &sessionInfoPtr->privateKey ); \
		if( cryptStatusError( status ) ) \
			retIntError()
#define FAULTACTION_SESSION_WRONGCERT_TLS_2 \
		krnlSendNotifier( sessionInfoPtr->privateKey, IMESSAGE_DESTROY ); \
		sessionInfoPtr->privateKey = faultParam1

/* Corrupt the finished MAC value */

#define FAULTACTION_SESSION_TLS_CORRUPT_FINISHED_1 \
		( ( BYTE * ) hashValues )[ 4 ]++
#define FAULTACTION_SESSION_TLS_CORRUPT_FINISHED_2 \
		( ( BYTE * ) hashValues )[ 4 ]--

/* Corrupt the hash value */

#define FAULTACTION_SESSION_BADSIG_HASH_TLS_1 \
...
#define FAULTACTION_SESSION_BADSIG_DATA_TLS_1 \
		( ( BYTE * ) keyData )[ 16 ]++
#define FAULTACTION_SESSION_BADSIG_DATA_TLS_2 \
		( ( BYTE * ) keyData )[ 16 ]--

/* Corrupt the server keyex signature */

#define FAULTACTION_SESSION_BADSIG_SIG_TLS_1 \
		( ( BYTE * ) dataPtr )[ 16 ]++

/* Corrupt the IV */

#define FAULTACTION_SESSION_TLS_CORRUPT_IV_1 \
		iv[ 2 ]++
...
/* Sometimes we want to skip code when we're fuzzing, typically because it's 
   unnecessary for the fuzzing and takes awhile to execute, or relies on
   data that hasn't been set up.  The following macros can be used to skip
   one line of code, and to skip the rest of the function.
   
   Note that FUZZ_SKIP_LINE() has to be used without a semicolon at the end */

#define FUZZ_SKIP_LINE()	if( 0 )		/* Used without semicolon */
#define FUZZ_SKIP_REMAINDER() \
							return( CRYPT_OK )
