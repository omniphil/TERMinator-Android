...
/* In multithreaded environments we need to use mutexes to protect the
   ...
   The types and macros that need to be declared to handle threading are:
	...
	THREAD_IS_CURRENT		-- Check if thread handle matches current thread
   ...
   This means that the ostensible thread functions sometimes take thread 
   arguments, sometimes semaphore arguments, and sometimes both:

	THREAD_CREATE: Thread and semaphore handles, may be the same.
	THREAD_EXIT: Semaphore handle to signal the thread exit, which 
				 THREAD_WAIT() waits on.
	THREAD_WAIT: Thread and semaphore handle, wait on the appropriate one.
	THREAD_CLOSE: Thread handle to close.
...
/* Thread management functions.  AMX threads require that the user allocate 
   the stack space for them, to handle this we use our own wrappers.  A 
   second problem with AMX threads is that there's no obvious way to pass an 
   argument to a thread.  In theory we could convey the information by 
   sending it via a mailbox, but this requires first conveying the mailbox 
   ID to the new task, which has the same problem.
...
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			{ \
			BYTE *threadStack = clAlloc( "Thread create",
										 EMBEDDED_STACK_SIZE ); \
			\
...
#define MUTEX_LOCK( name ) \
		if( acquire_sem_etc( krnlData->name##Mutex, 1, \
			...
			{ \
			if( !THREAD_IS_CURRENT( krnlData->name##MutexOwner ) ) \
				acquire_sem( krnlData->name##Mutex ); \
			else \
...
#define THREAD_WAIT( thread, sync, status ) \
								...
								status_t dummy; \
								\
								if( wait_for_thread( thread, &dummy ) != B_NO_ERROR ) \
									status = CRYPT_ERROR; \
								}
#define THREAD_CLOSE( thread )
...
/* To use resource-management wrappers for the ChorusOS thread functions,
   undefine the following */

/* #define CHORUS_THREAD_WRAPPERS */
...
/* Thread management functions.  ChorusOS threads require that the user 
   allocate the stack space for them, to handle this we use our own 
   wrappers.  A second problem with ChorusOS threads is that there's no easy 
   way to pass an argument to a thread, so we have to include it as a 
   "software register" value that the thread then obtains via threadLoadR().
...
/* Thread management functions.  eCOS threads require that the user allocate 
   the stack space for them, to handle this we use our own wrappers which 
   also provide access via a single scalar variable.  For synchronisation we 
   use semaphores, eCOS also provides condition variables for this purpose 
   but they require a user-managed mutex to synchronise access to them, 
   making them (at best) a primitive DIY semaphore.
...
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			{ \
			BYTE *threadData = clAlloc( "Thread create",
										sizeof( cyg_thread ) + \
...
#define THREAD_CLOSE( thread )	cyg_thread_delete( thread )
...
/* Thread management functions.  embOS threads require that the user 
   allocate the stack space for them, to handle this we use our own wrappers 
   which also provide access via a single scalar variable.
   ...
   The implementation of THREAD_IS_CURRENT() is somewhat ugly in that since 
   thread data storage is handled by the caller rather than being allocated 
   by the OS we end up with a mixture of 'struct' (if we're referring to a 
   block of storage for the thread data) and 'struct *' (if we're referring 
   to a thread handle returned from a function like OS_GetTaskID()).  
   THREAD_IS_CURRENT() takes advantage of the fact that the argument is 
   always a 'struct', which is rather ugly but the only other way to do it 
   would be to wrap the thread functions with our own code that dynamically 
   allocates thread data so that all handles are 'struct *'s */
...
/* Thread management functions.  FreeRTOS threads are generally sensible and
   ...
   To deal with this we divide the EMBEDDED_STACK_SIZE define by the width
   (word size), which is specified as configSTACK_DEPTH_TYPE.
   
   The checking for return values in THREAD_CREATE() looks a bit odd, 
   xSemaphoreTake() and xTaskCreate() have the same return type but 
   different names, pdTRUE for xSemaphoreTake() and pdPASS for xTaskCreate() 
   (pdPASS is defined to pdTRUE in projdefs.h) so the apparently dissimilar
   check for return codes is valid.  Unfortunately we can't enforce this 
   through a #if expression because they're defined in a way that can't be
   evaluated by the preprocessor */

#ifndef configSTACK_DEPTH_TYPE
  #error configSTACK_DEPTH_TYPE isnt defined.
#endif /* configSTACK_DEPTH_TYPE */

#define FREERTOS_STACK_SIZE \
		( EMBEDDED_STACK_SIZE / sizeof( configSTACK_DEPTH_TYPE ) )
...
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			...
			if( syncHandle != NULL ) \
				result = xSemaphoreTake( syncHandle, MUTEX_WAIT_TIME ); \
			if( result == pdTRUE ) \
				{ \
				result = xTaskCreate( function, "clibTask", FREERTOS_STACK_SIZE, \
...
#define THREAD_CLOSE( thread )	vTaskDelete( thread )
...
/* Indicate that Mongoose OS has non-scalar handles */
...
#define THREAD_CLOSE( thread )	NU_Delete_Task( thread )
...
#define THREAD_WAIT( thread, sync, status ) \
								KS_TestMutxW( sync ); \
								KS_CloseMutx( sync )
#define THREAD_CLOSE( thread )	XXXX_DeleteThread( thread )
...
#elif defined( __RiotOS__ )

/* To use resource-management wrappers for the RiotOS thread functions,
   undefine the following */

/* #define RIOTOS_THREAD_WRAPPERS */
...
#define THREAD_HANDLE			kernel_pid_t
#define MUTEX_HANDLE			rmutex_t
...
#define MUTEX_CREATE( name, status ) \
		...
		if( !krnlData->name##MutexInitialised ) \
			{ \
			rmutex_init( &krnlData->name##Mutex ); \
			krnlData->name##MutexInitialised = TRUE; \
			}
#define MUTEX_DESTROY( name ) \
		if( krnlData->name##MutexInitialised ) \
			{ \
			rmutex_lock( &krnlData->name##Mutex ); \
			rmutex_unlock( &krnlData->name##Mutex ); \
			krnlData->name##MutexInitialised = FALSE; \
			}
#define MUTEX_LOCK( name ) \
		rmutex_lock( &krnlData->name##Mutex )
...
/* RiotOS has a whole pile of braindamage around threading which means we 
   disable running our own threads by default via RIOTOS_THREAD_WRAPPERS.
   To create a thread, RiotOS threads require that the user allocate the 
   stack space for them, to handle this we use our own wrappers.  This 
   however is the most basic of the problems, the rest are much worse...
   
   There's no provision for semaphores, the closest equivalent is a 
   condition variable but this requires a user-provided mutex (so for
   example there's a "cond_wait( cond_t *cond, mutex_t *mutex )") which 
   both makes it unclear why there are condition variables in the first 
   place if you can just build them yourself with the mutex you have to use
   anyway, and doesn't allow them to be used without hauling around two
   variables instead of one.

   RiotOS states that a new thread shouldn't be created with the same 
   priority as an existing thread but there's no way to get the current 
   thread's priority, in theory we could use thread_get() to get the current
   thread's TCB and read out the 'priority' field but this seems like a bad
   thing to do, so we use a priority one higher than the priority of the main
   thread, THREAD_PRIORITY_MAIN. 
   
   RiotOS' thread_sleep() doesn't sleep for a given amount of time but 
   simply puts the thread to sleep indefinitely until something wakes it up
   again, and there's no kernel-based timer mechanism that we can use to
   build our own ("RIOT features a tickless, preemptive, priority based 
   scheduler") so there's no easy way to provide this unless the kernel adds
   timers at some point */
...
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			...
			\
			semaphore_init( &syncHandle ); \
			threadHandle = thread_create( threadData, \
										  THREAD_STACKSIZE_DEFAULT, \
										  THREAD_PRIORITY_MAIN - 1, 0, \
										  function, arg, "thread_name" ); \
			if( !pid_is_valid( threadHandle ) ) \
				{ \
				free( threadData ); \
				status = CRYPT_ERROR; \
				} \
...
#define THREAD_EXIT( sync )		semaphore_post( &sync ); \
								sched_task_exit()
#define THREAD_INITIALISER		0
#define THREAD_SELF()			thread_getpid()
#define THREAD_IS_CURRENT( thread )	\
								( ( thread ) == thread_getpid() )
#define THREAD_SLEEP( ms )		thread_sleep()	/* See comment above */
#define THREAD_YIELD()			thread_yield()
#define THREAD_WAIT( thread, sync, status ) \
								if( !semaphore_wait( &sync ) ) \
									status = CRYPT_ERROR; \
								semaphore_destroy( &sync )
#define THREAD_CLOSE( thread )	/* Doesn't seem to be any way to delete a thread */

/* Because of the problems with resource management of RiotOS threads and
   related metadata, we no-op them out unless we're using wrappers by
   ensuring that any attempt to spawn a thread inside cryptlib fails,
   falling back to the non-threaded alternative.  Note that cryptlib itself
   is still thread-safe, it just can't do its init in an internal background 
...
#define THREAD_CLOSE( thread )	smx_TaskDelete( thread )
...
#define THREAD_IS_CURRENT( thread )	\
								( ( thread ) == m2m_os_get_current_task_id() )
...
#define THREAD_CLOSE( thread )	tx_thread_delete( thread )
...
/* Thread management functions.  uC/OS threads require that the user 
   allocate the stack space for them, to handle this we use our own wrappers. 
   Because of the strict priority-based scheduling there's no way to perform 
   a yield, the best that we can do is sleep for 1ms, which is better than 
...
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			{ \
			OS_STK *threadStack = clAlloc( "Thread create", \
										   EMBEDDED_STACK_SIZE ); \
			\
...
#define THREAD_IS_CURRENT( thread )	\
  ...
  /* emscripten doesn't have a thread-yield mechanism, at the JS level 
	 there's emscripten_sleep_with_yield() but it's unclear how to interact
	 with this from the C level */
  #define THREAD_YIELD()
...
/* VxWorks tasks are exited using the standard ANSI exit() function rather
   ...
   The argument to the thread function is actually ten ints (with the 
   obligatory nonportable int == void * misuse) but all we care about is the 
   first one.  In addition since we're using it to carry a pointer we cheat 
   a bit and declare the thread function as taking a 'void *' rather than an 
   int.
...
#define THREADFUNC_DEFINE( name, arg )	void name( /*int*/ void *arg )
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			{ \
...
/* Thread management functions.  Win32 requires a C library-aware wrapper
   ...
   directly.  For some unknown reason this is defined as returning a 
   uintptr_t rather than a HANDLE, so we have to cast it when we assign it
   to a HANDLE.

   Windows doesn't provide any way to get the current thread's handle, the
   obvious GetCurrentThread() returns a pesudo-handle that Windows 
   interprets to mean "the current thread" rather than its actual handle,
   GetCurrentThreadId() returns a thread identifier which "uniquely 
   identifies the thread throughout the system" but isn't a handle, and
   something like OpenThread() creates a new handle rather than returning
   the existing one.
   
   Because of this we end up in a booby-trap situation where sometimes a 
   thread identifier is a handle and sometimes it's an ID.  In particular 
   THREAD_CREATE(), THREAD_WAIT(), and THREAD_CLOSE() use a thread handle 
   and THREAD_SELF() and THREAD_IS_CURRENT() use a thread ID.  This works
   because the former are only used in wait-for-thread functions via
   SEMAPHORE_INFO and THREAD_INFO while the latter are only used in 
   is-this-the-same-thread functions via OBJECT_INFO.
  ...
  #define THREADFUNC_DEFINE( name, arg ) \
	...
	#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
				...
				threadHandle = ( THREAD_HANDLE ) hThread; \
...
#define THREAD_IS_CURRENT( thread )	\
								( ( thread ) == ( THREAD_HANDLE ) GetCurrentThreadId() )
#define THREAD_SLEEP( ms )		Sleep( ms )
#if defined( __WIN32__ )
...
/* Debugging forms of various thread functions.  The first set just prints 
   diagnostics on run, the second set record mutex operations in a FIFO	
   queue for analysis.  Note that the first set require DEBUG_DIAG_ATOMIC()
   support in misc/debug.h */
...
#define MUTEX_CREATE( name, status ) \
		...
		if( !krnlData->name##CriticalSectionInitialised ) \
			{ \
			InitializeCriticalSection( &krnlData->name##CriticalSection ); \
			krnlData->name##LockCount = 0; \
			krnlData->name##CriticalSectionInitialised = TRUE; \
			}
#undef MUTEX_LOCK
#define MUTEX_LOCK( name ) \
		EnterCriticalSection( &krnlData->name##CriticalSection ); \
		krnlData->name##LockCount++; \
		DEBUG_DIAG_ATOMIC( "%lX locked %s, count %d -> %d", \
						   GetCurrentThreadId(), \
						   ( #name[ 5 ] == '1' ) ? "MUTEX_RANDOM" : \
						   ( #name[ 5 ] == '2' ) ? "MUTEX_RANDOMNONCE" : \
						   ( #name[ 5 ] == '3' ) ? "MUTEX_SCOREBOARD" : #name, \
						   krnlData->name##LockCount - 1, \
						   krnlData->name##LockCount )
#undef MUTEX_UNLOCK
#define MUTEX_UNLOCK( name ) \
		krnlData->name##LockCount--; \
		DEBUG_DIAG_ATOMIC( "%lX unlocked %s, count %d -> %d", \
						   GetCurrentThreadId(), \
						   ( #name[ 5 ] == '1' ) ? "MUTEX_RANDOM" : \
						   ( #name[ 5 ] == '2' ) ? "MUTEX_RANDOMNONCE" : \
						   ( #name[ 5 ] == '3' ) ? "MUTEX_SCOREBOARD" : #name, \
...
#define THREAD_YIELD() \
		threadYield(); \
		DEBUG_DIAG_ATOMIC( "%lX yielded", GetCurrentThreadId() )
...
/* Object handles.  Zephyr OS implements both mutexes and semaphores but
   the handles for those are non-scalar, one a struct k_mutex and the other 
   a struct k_sem (thread handles are scalar, a 'struct k_thread *'), so we 
...
/* Indicate that Zephyr OS has non-scalar handles */
...
#define MUTEX_CREATE( name, status ) \
		...
		if( !krnlData->name##MutexInitialised ) \
			{ \
			k_mutex_init( &krnlData->name##Mutex ); \
			krnlData->name##MutexInitialised = TRUE; \
			}
#define MUTEX_DESTROY( name ) \
		if( krnlData->name##MutexInitialised ) \
			{ \
			k_mutex_lock( &krnlData->name##Mutex, K_FOREVER ); \
			k_mutex_unlock( &krnlData->name##Mutex ); \
			krnlData->name##MutexInitialised = FALSE; \
			}
#define MUTEX_LOCK( name ) \
		k_mutex_lock( &krnlData->name##Mutex, K_FOREVER ); 
...
/* Thread management functions.  Zephyr threads require that the user 
   allocate the stack space for them, to handle this we use our own wrappers 
   which also provide access via a single scalar variable.

   There's no way to tell what the current thread's priority is, but on the
   assumption that we're being started from the main thread we use 
   MAIN_THREAD_PRIORITY as the new thread's priority.

   There doesn't seem to be any way to tell whether a thread has been 
   successfully created/started or not (!!), and the source code just 
   assumes via k_thread_create() -> z_setup_new_thread() -> arch_new_thread()
   that nothing will fail, so we always return CRYPT_OK for the thread 
   creation */

#define THREADFUNC_DEFINE( name, arg )	void name( void *arg, void *arg2, void *arg3 )
#define THREAD_CREATE( function, arg, threadHandle, syncHandle, status ) \
			{ \
			BYTE *threadData = clAlloc( "Thread create", \
										sizeof( struct k_thread ) + \
											EMBEDDED_STACK_SIZE ); \
			\
			k_sem_init( &syncHandle, 0, 1 ); \
			threadHandle = k_thread_create( threadData, \
											threadData + sizeof( struct k_thread ), \
											EMBEDDED_STACK_SIZE - \
													sizeof( struct k_thread ), \
											function, arg, NULL, NULL, \
											MAIN_THREAD_PRIORITY, 0, \
											K_NO_WAIT ); \
			status = CRYPT_OK; \
			}
#define THREAD_EXIT( sync )		k_sem_give( sync ); \
								return
#define THREAD_INITIALISER		0
#define THREAD_SELF()			
#define THREAD_IS_CURRENT( thread )	\
								( ( thread ) == k_current_get() )
#define THREAD_SLEEP( ms )		k_sleep( ms )
#define THREAD_YIELD()			k_yield()
#define THREAD_WAIT( thread, sync, status ) \
								if( k_sem_take( sync, K_FOREVER ) != 0 ) \
									status = CRYPT_ERROR; \
								else \
									k_sem_give( sync )
#define THREAD_CLOSE( thread )
...
#define THREAD_INITIALISER						0
#define THREAD_SELF()							0
#define THREAD_IS_CURRENT( thread )				TRUE
#define THREAD_SLEEP( ms )
#define THREAD_YIELD()
#define THREAD_WAIT( thread, sync, status )
#define THREAD_CLOSE( thread )
...
/* To initialise thread/mutex handles we need our own version of DUMMY_INIT,
  ...
  #define DUMMY_INIT_THREAD		= { 0 }
#else
  #define DUMMY_INIT_MUTEX		= ( MUTEX_HANDLE ) 0
  #define DUMMY_INIT_THREAD		= ( THREAD_HANDLE ) 0
