...
/* Check whether we've received a UDP packet from the same location as
   the previous one.  This is used to quickly weed out noise for UDP server
   ports which, being connectionless, can get packets from multiple sources
   as part of their normal operations */

#ifdef USE_EAP 

CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1, 2 ) ) \
static BOOLEAN isSameAddress( const struct sockaddr *addr1,
							  const struct sockaddr *addr2 )
	{
	assert( isReadPtr( addr1, sizeof( struct sockaddr ) ) );
	assert( isReadPtr( addr2, sizeof( struct sockaddr ) ) );
...
int ioWait( INOUT_PTR NET_STREAM_INFO *netStream, 
			IN_INT_Z const int timeout,
			IN_BOOL const BOOLEAN previousDataRead, 
...
static int readSocketFunction( INOUT_PTR NET_STREAM_INFO *netStream, 
	...
	*length = 0;
	...
	if( cryptStatusError( status ) )
		return( status );
	LOOP_MAX_REV_INITCHECK( bytesToRead = maxLength,
							bytesToRead > 0 && \
								( timeout <= 0 || \
								  !checkMonoTimerExpired( &timerInfo ) ) )
		{
		...
		ENSURES( LOOP_INVARIANT_MAX_REV_XXX( bytesToRead, 1, maxLength ) );
		...
		clearErrorState();
#ifdef USE_EAP
		if( TEST_FLAGS( netStream->nFlags,
						STREAM_NFLAG_DGRAM | STREAM_NFLAG_ISSERVER,
						STREAM_NFLAG_DGRAM | STREAM_NFLAG_ISSERVER ) )
			{
			SOCKADDR_STORAGE udpAddrInfo;
			SIZE_TYPE udpAddrlen = sizeof( SOCKADDR_STORAGE );

			/* Some systems don't support recv()/send() on UDP sockets so we 
			   have to synthesise the effect of recv() using recvfrom() */
			bytesRead = recvfrom( netStream->netSocket, bufPtr, bytesToRead, 0,
								  ( struct sockaddr * ) &udpAddrInfo, 
								  &udpAddrlen );
			if( bytesRead > 0 )
				{
				...
				if( !TEST_FLAG( netStream->nFlags, \
					...
					{
					REQUIRES( rangeCheck( udpAddrlen, 1, 
										  sizeof( SOCKADDR_STORAGE ) ) );
					memcpy( netStream->transportInfo, &udpAddrInfo, 
							udpAddrlen );
					netStream->transportInfoPayloadSize = udpAddrlen;
					}
				else
					{
					/* If we've got data from a source other than the one 
					   that we've already been talking to, ignore it and 
					   continue */
					if( netStream->transportInfoPayloadSize != udpAddrlen || \
						!isSameAddress( netStream->transportInfo, 
										( struct sockaddr * ) &udpAddrInfo ) )
						{
						DEBUG_PRINT(( "Received UDP data from different "
									  "address to previous data, "
									  "skipping...\n" ));
						continue;
						}
					}
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 4 ) ) \
static int writeSocketFunction( INOUT_PTR NET_STREAM_INFO *netStream, 
	...
	*length = 0;
	...
	if( cryptStatusError( status ) )
		return( status );
	LOOP_MAX_REV_INITCHECK( bytesToWrite = maxLength,
							bytesToWrite > 0 && \
								( timeout <= 0 || \
								  !checkMonoTimerExpired( &timerInfo ) ) )
		{
		...
		ENSURES( LOOP_INVARIANT_MAX_REV_XXX( bytesToWrite, 1, maxLength ) );
		...
		if( TEST_FLAGS( netStream->nFlags, 
			...
			{
			/* Some systems don't support recv()/send() on UDP sockets so we 
			   have to synthesise their effect using recvfrom()/sendto().
			   
			   The following code relies on the fact that as the server we 
			   call recvfrom() first to accept client messages, so the
			   transportInfo field is filled out by that ready for the 
			   sendto() call */
			bytesWritten = sendto( netStream->netSocket, bufPtr, bytesToWrite, 
								   MSG_NOSIGNAL, netStream->transportInfo, 
...
STDC_NONNULL_ARG( ( 1 ) ) \
void setAccessMethodTCPReadWrite( INOUT_PTR NET_STREAM_INFO *netStream )
	{
	assert( isWritePtr( netStream, sizeof( NET_STREAM_INFO ) ) );
