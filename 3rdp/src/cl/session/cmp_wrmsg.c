...
static int writeRequestBody( INOUT_PTR STREAM *stream,
...
static int writeResponseBodyHeader( INOUT_PTR STREAM *stream, 
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int writeEncryptedResponseBody( INOUT_PTR STREAM *stream,
									   INOUT_PTR SESSION_INFO *sessionInfoPtr,
									   const CMP_PROTOCOL_INFO *protocolInfo )
	{
	MESSAGE_DATA msgData;
	...
	if( cryptStatusError( status ) )
		...
		retExtErr( status,
				   ( status, SESSION_ERRINFO, &localErrorInfo, 
					 "Couldn't wrap response data for '%s'",
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int writeResponseBody( INOUT_PTR STREAM *stream,
							  INOUT_PTR SESSION_INFO *sessionInfoPtr,
							  const CMP_PROTOCOL_INFO *protocolInfo )
	{
	MESSAGE_DATA msgData;
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2, 3 ) ) \
static int writeConfBody( INOUT_PTR STREAM *stream,
						  INOUT_PTR SESSION_INFO *sessionInfoPtr,
						  const CMP_PROTOCOL_INFO *protocolInfo )
	{
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writePKIConfBody( INOUT_PTR STREAM *stream,
...
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
static int writeGenMsgRequestBody( INOUT_PTR STREAM *stream,
							STDC_UNUSED SESSION_INFO *sessionInfoPtr,
							STDC_UNUSED const CMP_PROTOCOL_INFO *protocolInfo )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
static int writeGenMsgResponseBody( INOUT_PTR STREAM *stream,
							INOUT_PTR SESSION_INFO *sessionInfoPtr,
							STDC_UNUSED const CMP_PROTOCOL_INFO *protocolInfo )
	{
...
static int writeErrorBody( INOUT_PTR STREAM *stream,
						   STDC_UNUSED SESSION_INFO *sessionInfoPtr,
						   const CMP_PROTOCOL_INFO *protocolInfo )
	{
	int statusInfoLength;
	...
	statusInfoLength = sizeofPkiStatusInfo( protocolInfo->status, 
											protocolInfo->pkiFailInfo );
	ENSURES( isShortIntegerRangeNZ( statusInfoLength ) );
...
WRITEMESSAGE_FUNCTION getMessageWriteFunction( IN_ENUM( CMPBODY ) \
	...
	REQUIRES_N( isBooleanValue( isServer ) );
	...
	if( isServer )
		...
		LOOP_SMALL( i = 0, 
			...
			ENSURES_N( LOOP_INVARIANT_SMALL( i, 0, 
											FAILSAFE_ARRAYSIZE( messageWriteServerTable, \
																MESSAGEWRITE_INFO ) - 1 ) );
		...
		LOOP_SMALL( i = 0, 
			...
			{
			ENSURES_N( LOOP_INVARIANT_SMALL( i, 0, 
											 FAILSAFE_ARRAYSIZE( messageWriteClientTable, \
																 MESSAGEWRITE_INFO ) - 1 ) );
