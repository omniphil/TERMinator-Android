...
int addRandomData( INOUT_PTR void *statePtr, 
				   IN_BUFFER( valueLength ) const void *value, 
				   IN_LENGTH_SHORT const int valueLength );
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int addRandomLong( INOUT_PTR void *statePtr, const long value );
RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int endRandomData( INOUT_PTR void *statePtr, \
...
/* Prototypes for functions in random.c.  Since the RANDOM_INFO struct isn't 
   visible at this point, we have to use a forward declaration for it, and
   also estimate its size at a minimum of 256 bytes.  This will give an
   analyser warning for misc/random.c, which uses the actual size of the
   struct */

struct RI;

CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int initRandomInfo( INOUT_BUFFER_FIXED( 256 ) struct RI *randomInfoPtr );
STDC_NONNULL_ARG( ( 1 ) ) \
void endRandomInfo( INOUT_BUFFER_FIXED( 256 ) struct RI *randomInfoPtr );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int addEntropyData( INOUT_PTR struct RI *randomInfoPtr, 
					IN_BUFFER( length ) const void *buffer, 
					IN_DATALENGTH const int length );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1 ) ) \
int addEntropyQuality( INOUT_PTR struct RI *randomInfoPtr, 
					   IN_RANGE( 1, 100 ) const int quality );
CHECK_RETVAL STDC_NONNULL_ARG( ( 1, 2 ) ) \
int getRandomData( INOUT_PTR struct RI *randomInfoPtr, 
